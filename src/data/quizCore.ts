...
function buildBattleContextQuestions(params: BattleQuizParams): WeightedBattleQuizQuestion[] {
  ...
  return [
    ...
    makeQuestion(
      ...
      makeChoices("The foe counters immediately", ["You heal for free", "The map changes"]),
      1.15,
    ),
    makeQuestion(
      "level-advantage",
...
function buildBattleSourceQuestions(params: BattleQuizParams): WeightedBattleQuizQuestion[] {
...
function buildPokemonKnowledgeQuestions(): WeightedBattleQuizQuestion[] {
  return pokemonQuizEntries.map((entry) =>
    makeQuestion(
      entry.id,
      entry.prompt,
...
export function buildWeightedBattleQuizQuestions(
  ...
  return [
    ...buildBattleContextQuestions(params),
    ...buildBattleSourceQuestions(params),
    ...buildPokemonKnowledgeQuestions(),
  ];
}
